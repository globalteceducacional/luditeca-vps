...
export async function registerLibrasLessonRoutes(app) {
    app.get('/libras-lessons', { preHandler: requireCmsEditor }, async (request, reply) => {
    ...
    app.patch('/libras-lessons/reorder', { preHandler: requireCmsEditor }, async (request, reply) => {
        const body = request.body;
        if (!Array.isArray(body.ids) || body.ids.length === 0) {
            return reply.code(400).send({ error: 'ids (array) é obrigatório.' });
        }
        const ids = body.ids.map((id) => String(id));
        await prisma.$transaction(ids.map((id, index) => prisma.librasLesson.update({
            where: { id },
    ...
    app.get('/libras-lessons/:id', { preHandler: requireCmsEditor }, async (request, reply) => {
        const row = await prisma.librasLesson.findUnique({
            where: { id: request.params.id },
        });
        if (!row)
            return reply.code(404).send({ error: 'Lição LIBRAS não encontrada.' });
        return reply.send(librasLessonToApi(row));
    });
    app.post('/libras-lessons', { preHandler: requireCmsEditor }, async (request, reply) => {
        const body = request.body;
        const word = String(body.word || '').trim();
        if (!word)
            return reply.code(400).send({ error: 'word é obrigatório.' });
        const row = await prisma.librasLesson.create({
            data: {
                ...
                quizQuestion: body.quiz_question != null || body.quizQuestion != null
                    ? String(body.quiz_question ?? body.quizQuestion)
                    : null,
                quizOptions: Array.isArray(body.quiz_options ?? body.quizOptions)
                    ? (body.quiz_options ?? body.quizOptions)
                    : undefined,
                quizCorrect: body.quiz_correct != null || body.quizCorrect != null
    ...
    app.patch('/libras-lessons/:id', { preHandler: requireCmsEditor }, async (request, reply) => {
        const body = request.body;
        const data = {};
        if (body.word !== undefined)
            data.word = String(body.word).trim();
        ...
        if (body.quiz_options !== undefined || body.quizOptions !== undefined) {
            ...
            data.quizOptions = Array.isArray(arr) ? arr : undefined;
        }
    ...
    app.delete('/libras-lessons/:id', { preHandler: requireCmsEditor }, async (request, reply) => {
