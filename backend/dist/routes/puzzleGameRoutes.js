...
import { parseBoolean, parsePagination, parsePuzzlePieceCount, validatePuzzlePayload, } from '../lib/contentTypes.js';
import { prisma } from '../lib/prisma.js';
import { requireCmsEditor } from '../plugins/auth.js';
const MODULE = 'MOD:cms';
export async function registerPuzzleGameRoutes(app) {
    app.get('/puzzle-games', { preHandler: requireCmsEditor }, async (request, reply) => {
    ...
    app.get('/puzzle-games/:id', { preHandler: requireCmsEditor }, async (request, reply) => {
        const row = await prisma.puzzleGame.findUnique({
            where: { id: request.params.id },
        });
        if (!row)
            return reply.code(404).send({ error: 'Quebra-cabeça não encontrado.' });
        return reply.send(puzzleGameToApi(row));
    });
    app.post('/puzzle-games', { preHandler: requireCmsEditor }, async (request, reply) => {
    ...
    app.patch('/puzzle-games/:id', { preHandler: requireCmsEditor }, async (request, reply) => {
        const body = request.body;
        const data = {};
    ...
    app.delete('/puzzle-games/:id', { preHandler: requireCmsEditor }, async (request, reply) => {
