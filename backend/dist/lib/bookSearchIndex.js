...
export function buildSearchIndexText(parts) {
...
export function parseCatalogStringArrayFromBody(v) {
    if (v === undefined)
        return undefined;
    if (v === null)
        return [];
    if (Array.isArray(v))
        return jsonStringArray(v);
    if (typeof v === 'string') {
        return v
            .split(/[,;]/)
            .map((s) => s.trim())
...
export async function persistBookSearchIndex(bookId) {
