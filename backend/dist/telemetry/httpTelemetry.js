...
export function registerHttpTelemetry(app) {
    ...
    app.addHook('onResponse', async (request, reply) => {
        ...
        if (pathOnly === '/telemetry/client' && status < 400)
            return;
        ...
        const persistError = status >= 500 || status === 429 || (status >= 400 && isMedia) || (status >= 400 && pathOnly === '/telemetry/client');
        if (!persistSlow && !persistError)
            return;
