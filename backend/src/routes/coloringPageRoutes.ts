...
export async function registerColoringPageRoutes(app: FastifyInstance) {
  app.get(
    '/coloring-pages',
    ...
    async (request, reply) => {
      ...
      return reply.send(
        listEnvelope(rows.map(coloringPageToApi), total, limit, offset),
      );
    },
  );
  ...
  app.get<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      ...
      if (!row) return reply.code(404).send({ error: 'Pintura não encontrada.' });
        ...
        data: {
          ...
          imageUrl:
            body.image_url != null || body.imageUrl != null
              ...
              : null,
          defaultId:
            body.default_id != null || body.defaultId != null
              ? String(body.default_id ?? body.defaultId).trim() || null
              : null,
          svgType:
            body.svg_type != null || body.svgType != null
  ...
  app.patch<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      ...
      if (!prev) return reply.code(404).send({ error: 'Pintura não encontrada.' });
