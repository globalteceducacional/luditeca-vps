...
import { parseJsonMap, type XpMeta, XP_META_PROGRESS_KEY } from './xpRewards.js';
...
function parseBadgeIds(badges: unknown): string[] {
...
export function evaluateAchievements(input: {
  ...
  for (const def of ACHIEVEMENT_DEFINITIONS) {
    if (def.comingSoon || owned.has(def.id)) continue;
