...
import { buildProfileStats } from './profileStats.js';
...
export function achievementsCatalogPayload(
  badges: unknown,
  stats: ReturnType<typeof buildProfileStats>,
) {
  const owned = new Set(
    Array.isArray(badges) ? badges.map((b) => String(b)) : [],
  );
  ...
  return ACHIEVEMENT_DEFINITIONS.map((def) => {
    ...
    const progress =
      def.threshold > 0 ? Math.min(1, current / def.threshold) : 0;
